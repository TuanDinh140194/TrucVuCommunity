...
const EventsConfigure: React.FC<Props> = props => {
  ...
  const {setQuestion, setIsModalOpen, setAnswers} = useContext(PollContext);
  React.useEffect(() => {
  ...
  useEffect(() => {
    ...
    events.on(controlMessageEnum.initiatePoll, async ({payload}) => {
      const {question, answers} = await JSON.parse(payload);
    ...
    return () => {
      ...
      events.off(controlMessageEnum.initiatePoll);
